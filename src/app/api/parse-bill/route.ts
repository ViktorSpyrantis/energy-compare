...
const EXTRACTION_PROMPT = `Εξέτασε αυτό το αρχείο/έγγραφο.

Εξήγαγε τα παρακάτω στοιχεία και επέστρεψε ΜΟΝΟ ένα JSON αντικείμενο, χωρίς markdown ή άλλο κείμενο:

{
  "isNotBill": <true αν ΔΕΝ είναι λογαριασμός ενέργειας (ρεύματος ή αερίου) - boolean>,
  "isGasBill": <true αν είναι λογαριασμός φυσικού αερίου, false αν είναι ηλεκτρικού ρεύματος - boolean>,
...
Οδηγίες:
- isNotBill: Αν το έγγραφο ΔΕΝ είναι λογαριασμός ενέργειας (π.χ. είναι φωτογραφία, γενικό έγγραφο, τιμολόγιο άλλου τύπου, λογαριασμός νερού/τηλεφώνου, ή οτιδήποτε άλλο που δεν σχετίζεται με ηλεκτρικό ρεύμα ή φυσικό αέριο), βάλε true. Αν είναι λογαριασμός ρεύματος ή αερίου, βάλε false.
- isGasBill: Αν ο λογαριασμός αφορά φυσικό αέριο (π.χ. αναφέρει "Φυσικό Αέριο", "φυσ. αέριο", "αέριο", "θέρμανση", μονάδες σε m³ ή kWh θερμικής ενέργειας, πάροχο φυσικού αερίου όπως ΕΠΑ, Φυσικό Αέριο Ελληνική Εταιρεία Ενέργειας, Ζένιθ Αέριο, Attiki Gas, κ.ά.), βάλε true. Αν είναι λογαριασμός ηλεκτρικού ρεύματος, βάλε false.
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result2: ParsedBill = {
      isNotBill: extracted.isNotBill === true,
      isGasBill: extracted.isGasBill === true,
      kwh: monthlyKwh,
